...
class Fun(commands.Cog):
	...
	@commands.command()
	async def balance(self, ctx, discord.User=None):
		with open('slots.json', 'r') as f:
			data = json.load(f)
			if discord.User = None:
				await ctx.send(f'{ctx.author.mention}s balance is {data[str(ctx.author.id)]["balance"]')
			else:
				await ctx.send(f'{user.mention}s balance is {data[str(user.id)]["balance"]')
